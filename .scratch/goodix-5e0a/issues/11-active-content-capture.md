...
2. **Payload `05` produces pure blanks & reveals session state divergence:**
   ...
   *Key realization*: The exact same 05-payload bytes return a 10638-byte reply in
   Windows and a 7684-byte reply on Linux. Same payload, different reply size
   proves the divergence is **session state**, not steady-state payload bytes. Our
   activation establishes a working-but-degraded session; Windows' cold init
   (reset, config blob, register writes, TLS handshake — absent from the warm
   capture) establishes the full one. This cold init divergence is the prime
   suspect for blank content.
3. **Payload `01` is the proven content path on Linux:**
...
4. **Register 0x022c write is eliminated:**
   ...
   Windows never writes `0x022c` in the warm capture. Kept bypassed.
5. **Highest-leverage external action (Windows):**
   Run cold-plug USBPcap capture before device first-open (reboot with capture
   running, or disable+re-enable device in Device Manager during capture), then
   enroll. The initial ~200 packets contain the true init sequence (cold reset,
   true config blob, initial register writes, TLS negotiation).
...
- [ ] Phase 2 (60s press-and-hold): stages advance within seconds of touch when content is present.
- [ ] (pending-content) Journal shows non-zero ridge content (`nonzero > 0`, `active > 0`).
- [ ] (pending-content) Multi-stage enrollment completes; verify matches.
*(Note: Criteria 3–4 are pending cold-plug init capture if the degraded session state prevents ridge formation under 01-payload)*
