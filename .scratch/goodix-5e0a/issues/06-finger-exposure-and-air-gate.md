...
- [x] A full multi-stage enroll completes on a real finger.
- [x] Daemon logs show minutiae extracted from enrollment frames (no zero-minutiae templates stored).
- [x] Air-only frames are rejected before template storage — air can never complete an enroll.
- [x] `fprintd-verify` matches the enrolled finger.
