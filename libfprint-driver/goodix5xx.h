 ...
 * process_raw_frame (preferred: it sees the raw 12-bit ADC raster directly,
 * as goodix5e0a does) and override change_state/deactivate for its own scan
 * lifecycle, in which case the base scan stages never run.
 *
 * @par Neither hook takes ownership of its input: the base frees the decoded
 * raster once the hook returns. Return NULL to reject the frame — the scan is
 * then failed rather than reported with a NULL image.
 *
