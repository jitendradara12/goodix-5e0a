...
// ================= TICKET 79 RESIDUAL-RANGE FRAME PROXY =================
// Offline residual_range dynamic-range probe (no driver changes, no hardware).
//
// Copy of the ticket-78 harness head (lines 1-896: pristine PE loader + P2/P5
// PGM parsers) with a new tail: per-frame residual_range (residual_max -
// residual_min from the driver-faithful 3x3 local-contrast path at the
// as-shipped shaping lc-g1.0-m128) + Milan getQuality + identifyImage match
// score vs legacy-experiments/milan_dense_pad.tpl (finger A).
//
// Purpose: replace Milan getQuality with residual_range as the best-of-N
// burst rank, validated on REAL LIVE BURST frames. Per the ticket, do NOT
// tune the threshold on synthetic frames alone: with zero live_burst files
// present this probe reports reference values and exits INCONCLUSIVE
// (exit 3). Only with real live_burst frames does it evaluate the
// 394..704 separation window with FAR intact.
//
// Usage:
//   gcc -O2 -o /tmp/opencode/probe79 legacy-experiments/test_residual_range_proxy.c -lm
//   timeout 120 /tmp/opencode/probe79 [live_burst_1.pgm live_burst_2.pgm ...]
// With no argv, probes candidate paths legacy-experiments/live_burst_*.pgm
// (and experiments/live_burst_*.pgm) plus synthetic references.
...
// residual = pix - 3x3 local mean, out = 128 + residual * 1.0, clamped.
// Reports residual min/max for the dynamic-range rank.
static void normalize_driver_shaping(const u16 *pix, u8 *out_bytes,
    ...
    for (int i = 0; i < GOODIX_FRAME_SIZE; i++) {
        int val = (int)roundf(128.0f + residual[i] * 1.0f);
        if (val < 0) val = 0;
        if (val > 255) val = 255;
        out_bytes[i] = (u8)val;
    }
    if (out_min) *out_min = res_min;
    if (out_max) *out_max = res_max;
}
...
int main(int argc, char **argv) {
    ...
    fprintf(stderr, "=== Ticket 79: Residual-Range Frame Proxy (offline, no driver changes) ===\n");
    fprintf(stderr, "Target DLL: %s\n", g_dllpath);
    fprintf(stderr, "Shaping: lc-g1.0-m128 (driver as-shipped); threshold window under test: 394..704\n");

    register_all_shims();
    if (load_pe() != 0) {
        fprintf(stderr, "[INCONCLUSIVE-because-dll-absent] Failed to load DLL\n");
        return 1;
    }
    ...
    if (!getAlgorithmVersion || !ppp_param_init || !getQuality ||
        !templateUnPack || !templateDelete || !identifyImage) {
        fprintf(stderr, "[INCONCLUSIVE-because-missing-export] Required exports missing\n");
        return 1;
    }
    ...
    // ---- Load ticket-72 master template (finger A) ----
    const char *tpl_paths[] = {
        "legacy-experiments/milan_dense_pad.tpl",
        "experiments/milan_dense_pad.tpl",
        NULL,
    };
    const char *tpl_used = NULL;
    u8 *tpl_buf = NULL;
    long tpl_len = 0;
    for (int i = 0; tpl_paths[i]; i++) {
        FILE *tf = fopen(tpl_paths[i], "rb");
        if (!tf) continue;
        fseek(tf, 0, SEEK_END);
        tpl_len = ftell(tf);
        fseek(tf, 0, SEEK_SET);
        tpl_buf = malloc(tpl_len);
        if (tpl_buf && fread(tpl_buf, 1, tpl_len, tf) == (size_t)tpl_len) {
            tpl_used = tpl_paths[i];
            fclose(tf);
            break;
        }
        if (tpl_buf) { free(tpl_buf); tpl_buf = NULL; }
        fclose(tf);
    }
    if (!tpl_buf) {
        fprintf(stderr, "[INCONCLUSIVE-because-template-missing] milan_dense_pad.tpl absent\n");
        return 1;
    }
    void *gallery = NULL;
    int unpack_res = templateUnPack(tpl_buf, (int)tpl_len, NULL, &gallery);
    fprintf(stderr, "[unpack] templateUnPack(%ld bytes from %s) returned %d, handle=%p\n",
            tpl_len, tpl_used, unpack_res, gallery);
    if (unpack_res != 0 || !gallery) {
        fprintf(stderr, "[INCONCLUSIVE-because-template-unpack-failed]\n");
        return 1;
    }
    void *active_templates[1] = { gallery };

    // ---- Frame list: argv override, else candidate live_burst paths + synthetic refs ----
    // Live candidates cover both legacy-experiments/ (real dir) and experiments/
    // (ticket text) prefixes and common 4-frame burst namings.
    const char *live_candidates[] = {
        "legacy-experiments/live_burst_01.pgm",
        "legacy-experiments/live_burst_02.pgm",
        "legacy-experiments/live_burst_03.pgm",
        "legacy-experiments/live_burst_04.pgm",
        "legacy-experiments/live_burst_00.pgm",
        "legacy-experiments/live_burst_1.pgm",
        "legacy-experiments/live_burst_2.pgm",
        "legacy-experiments/live_burst_3.pgm",
        "legacy-experiments/live_burst_4.pgm",
        "legacy-experiments/live_burst_press_1.pgm",
        "legacy-experiments/live_burst_press_2.pgm",
        "legacy-experiments/live_burst_press_3.pgm",
        "legacy-experiments/live_burst_press_4.pgm",
        ...
        "legacy-experiments/live_burst_tap_1.pgm",
        "legacy-experiments/live_burst_tap_2.pgm",
        "legacy-experiments/live_burst_tap_3.pgm",
        "legacy-experiments/live_burst_tap_4.pgm",
        "experiments/live_burst_01.pgm",
        "experiments/live_burst_02.pgm",
        "experiments/live_burst_03.pgm",
        "experiments/live_burst_04.pgm",
        NULL,
    };
    struct {
        const char *path;
        int is_live;      // 1 = real live burst frame (threshold population)
        int expect_match; // vs finger-A template: live press genuine=1; refs below
    } plan[64];
    int n_plan = 0;

    if (argc > 1) {
        // Explicit argv files are treated as live burst frames in order.
        for (int i = 1; i < argc && n_plan < 60; i++) {
            plan[n_plan].path = argv[i];
            plan[n_plan].is_live = 1;
            plan[n_plan].expect_match = -1; // unknown finger until enrolled live
            n_plan++;
        }
    } else {
        for (int i = 0; live_candidates[i] && n_plan < 48; i++) {
            if (file_exists(live_candidates[i])) {
                plan[n_plan].path = live_candidates[i];
                plan[n_plan].is_live = 1;
                plan[n_plan].expect_match = -1;
                n_plan++;
            }
        }
    }
    ...
    // Synthetic references (NOT the threshold population — harness sanity only).
    static const char *ref_paths[] = {
        "legacy-experiments/live_dense_pad.pgm",          // synthetic dense genuine A
        "legacy-experiments/live_dense_pad_seed68.pgm",   // synthetic dense impostor B
        "legacy-experiments/fingerprint.pgm",             // sparse impostor
        "legacy-experiments/clear-0.pgm",                 // blank
        NULL,
    };
    static const int ref_expect[] = { 1, 0, 0, 0 };
    static const char *ref_labels[] = {
        "ref: synthetic dense genuine A",
        "ref: synthetic dense impostor B",
        "ref: sparse fingerprint.pgm",
        "ref: blank clear-0",
    };
    (void)ref_labels;

    fprintf(stderr, "\n=== Per-frame residual_range @ lc-g1.0-m128 + getQuality + identifyImage ===\n");
    fprintf(stderr, "%-44s | %8s | %8s | %8s | q ov | score | match | det\n",
            "frame", "res_min", "res_max", "range");
    fprintf(stderr, "---------------------------------------------+----------+----------+----------+-------+-------+-------+---------\n");

    u8 shaped[GOODIX_FRAME_SIZE];
    int genuine_ok = 1, impostor_ok = 1;
    float ref_range[4] = {-1, -1, -1, -1};
    int n_ref = 0;
    float live_range[48];
    int live_score[48];
    int n_live_done = 0;

    // Helper macro body inlined: process one P2 file.
    for (int pi = 0; pi < n_plan; pi++) {
        u16 raw[GOODIX_FRAME_SIZE];
        int pw = 0, ph = 0;
        if (read_p2_pgm(plan[pi].path, raw, &pw, &ph) != 0) {
            fprintf(stderr, "%-44.44s | UNREADABLE (skip)\n", plan[pi].path);
            continue;
        }
        int active = 0;
        for (int i = 0; i < GOODIX_FRAME_SIZE; i++) if (raw[i] > 30) active++;
        float rmin = 0, rmax = 0;
        normalize_driver_shaping(raw, shaped, &rmin, &rmax);
        float range = rmax - rmin;

        GoodixImage probe;
        make_goodix_image(&probe, shaped, sensor_type);
        u32 qout[2] = {0xdeadbeef, 0xdeadbeef};
        getQuality(&probe, qout);

        GoodixImage probe2;
        make_goodix_image(&probe2, shaped, sensor_type);
        int matched_idx = -999, match_score = -999;
        u32 details[2] = {0};
        identifyImage(&probe2, NULL, active_templates, 1,
                      &matched_idx, &match_score, details, 0, 0, NULL, 0);
        int is_match = (matched_idx == 0 && match_score > 0);

        fprintf(stderr, "%-44.44s | %+8.1f | %+8.1f | %8.1f | %2u %2u | %5d | %-5s | [%2u,%2u] active=%d qout=[%u,%u]\n",
                plan[pi].path, rmin, rmax, range,
                probe.quality, probe.overlap,
                match_score >= 0 ? match_score : 0,
                is_match ? "MATCH" : "NO",
                details[0], details[1], active, qout[0], qout[1]);
        if (n_live_done < 48) {
            live_range[n_live_done] = range;
            live_score[n_live_done] = (match_score >= 0 ? match_score : 0);
            n_live_done++;
        }
    }

    for (int ri = 0; ref_paths[ri]; ri++) {
        u16 raw[GOODIX_FRAME_SIZE];
        int pw = 0, ph = 0;
        if (read_p2_pgm(ref_paths[ri], raw, &pw, &ph) != 0) {
            fprintf(stderr, "[warn] %s missing, skipping\n", ref_paths[ri]);
            continue;
        }
        int active = 0;
        for (int i = 0; i < GOODIX_FRAME_SIZE; i++) if (raw[i] > 30) active++;
        ...
        identifyImage(&probe2, NULL, active_templates, 1,
                      &matched_idx, &match_score, details, 0, 0, NULL, 0);
        int is_match = (matched_idx == 0 && match_score > 0);
        int correct = (is_match == ref_expect[ri]);
        if (ref_expect[ri]) { if (!correct) genuine_ok = 0; }
        else { if (!correct) impostor_ok = 0; }
        if (n_ref < 4) ref_range[n_ref++] = range;

        fprintf(stderr, "%-44.44s | %+8.1f | %+8.1f | %8.1f | %2u %2u | %5d | %-5s | [%2u,%2u] active=%d qout=[%u,%u] %s\n",
                ref_paths[ri], rmin, rmax, range,
                probe.quality, probe.overlap,
                match_score >= 0 ? match_score : 0,
                is_match ? "MATCH" : "NO",
                details[0], details[1], active, qout[0], qout[1],
    ...
    fprintf(stderr, "---------------------------------------------+----------+----------+----------+-------+-------+-------+---------\n");
    fprintf(stderr, "[far] synthetic-ref genuine-100 intact: %s; impostor/blank-0 intact: %s\n",
            genuine_ok ? "yes" : "NO", impostor_ok ? "yes" : "NO");
    if (n_ref == 4)
        fprintf(stderr, "[ref ranges] dense-A=%.1f dense-B=%.1f sparse=%.1f blank=%.1f (ticket-78: 727/704/394/0)\n",
                ref_range[0], ref_range[1], ref_range[2], ref_range[3]);

    templateDelete(gallery);
    free(tpl_buf);

    if (n_live_done == 0) {
        fprintf(stderr, "\n[VERDICT: INCONCLUSIVE-because-no-live-burst] zero live_burst_*.pgm files found; ");
        fprintf(stderr, "threshold NOT tuned on synthetic refs. Save Step-0 burst, re-run with paths.\n");
        return 3;
    }

    // Live separation check: every live frame that matches (score>0) must sit
    // at/above the threshold while sparse/blank refs sit below, with FAR intact.
    // Lower bound is max(sparse, blank) ONLY: dense-B is a different finger
    // (identity, not clarity — ticket-78 finding #3: quality/range is
    // clarity-only, never across fingers), so it is reported as context and
    // never gates the clarity threshold. Optional argv template mode: when
    // the Step-0 burst is a different finger than template-A, live scores are
    // 0 and the range distribution is still reported match-decoupled.
    // Candidate threshold is reported, not pinned.
    float live_min_match = 1e9f, live_max = -1e9f;
    for (int i = 0; i < n_live_done; i++) {
        if (live_range[i] > live_max) live_max = live_range[i];
        if (live_score[i] > 0 && live_range[i] < live_min_match) live_min_match = live_range[i];
    }
    float ref_max_neg = -1e9f; // sparse/blank only (indices 2,3 when all refs present)
    for (int i = 2; i < n_ref; i++) if (ref_range[i] > ref_max_neg) ref_max_neg = ref_range[i];
    float dense_b_range = (n_ref > 1) ? ref_range[1] : -1e9f; // context only, never gates
    if (live_min_match < 1e8f)
        fprintf(stderr, "[live] n=%d max_range=%.1f min_matching_range=%.1f sparse_blank_max=%.1f dense-B(context)=%.1f\n",
                n_live_done, live_max, live_min_match, ref_max_neg, dense_b_range);
    else
        fprintf(stderr, "[live] n=%d max_range=%.1f min_matching_range=NONE-matched (finger mismatch? enroll a live template) sparse_blank_max=%.1f dense-B(context)=%.1f\n",
                n_live_done, live_max, ref_max_neg, dense_b_range);
    if (genuine_ok && impostor_ok && live_min_match < 1e8f && live_min_match > ref_max_neg &&
        ref_max_neg < 704 && live_min_match > 394)
        fprintf(stderr, "[VERDICT: CONFIRMED] residual_range separates live matching frames from blanks/sparse with FAR intact; candidate threshold in (%.1f, %.1f).\n",
                ref_max_neg, live_min_match);
    else
        fprintf(stderr, "[VERDICT: FALSIFIED-or-OVERLAP] ranges interleave or FAR broke; do NOT wire; consider fusion/enrollment-side next.\n");
    (void)n_live;
    (void)live_candidates;
    return 0;
}
