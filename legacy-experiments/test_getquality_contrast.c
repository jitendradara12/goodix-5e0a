...
// ================= TICKET 78 CONTRAST-RESPONSE PROBE =================
// Offline getQuality contrast-response probe (no driver changes, no hardware).
//
// Feeds saved 5e0a raw frames (experiments/*.pgm) through Milan getQuality
// while sweeping the normalization variants the driver could plausibly feed
// it — contrast gain {1.0, 1.5, 2.0} at midpoint 128, midpoint {100, 160} at
// gain 1.5, and raw-vs-normalized input (12->8 shift, no local contrast) —
// and records quality/overlap/qout[2] alongside identifyImage match scores
// against experiments/milan_dense_pad.tpl (ticket-72 template, finger A).
//
// Successor decision:
//   CONFIRM (shaping found): some gain reports spread (good >= 10,
//     poor/blank = 0) with genuine 100 / impostor 0 intact.
//   FALSIFY (dead end): quality 0/0 and qout flat across all gains and frames
//     while identifyImage still separates 100-vs-0.

// Driver-faithful local-contrast normalization with explicit gain/midpoint.
// Matches libfprint-driver/goodix5e0a.c goodix5e0a_normalize_raw_frame and
// process_raw_frame: residual = pix - 3x3 local mean, out = midpoint +
// residual * gain, clamped. Residual stats are reported for the
// dynamic-range successor (residual_range correlation).
static void normalize_gain_midpoint(const u16 *pix, u8 *out_bytes,
                                    float gain, float midpoint,
    ...
    for (int i = 0; i < GOODIX_FRAME_SIZE; i++) {
        int val = (int)roundf(midpoint + residual[i] * gain);
        if (val < 0) val = 0;
        if (val > 255) val = 255;
        out_bytes[i] = (u8)val;
    }
...
// Raw-vs-normalized input: direct 12-bit -> 8-bit shift, no local contrast.
static void raw12_to_8bit(const u16 *pix, u8 *out_bytes) {
    for (int i = 0; i < GOODIX_FRAME_SIZE; i++) {
        u32 v = pix[i] >> 4;
        if (v > 255) v = 255;
        out_bytes[i] = (u8)v;
...
typedef struct {
    ...
    float gain;
    float midpoint;
    int is_raw; // 1 = raw shift, no local contrast
} Variant;

int main(int argc, char **argv) {
    g_dllpath = "/home/sastauser/goodix-27c6-5e0a-re/drivers/GoodixEngineAdapter.dll";
    fprintf(stderr, "=== Ticket 78: Offline getQuality Contrast-Response Probe ===\n");
    ...
    // ---- Load saved raw frames (12-bit P2 PGM) ----
    ...
    int pw = 0, ph = 0;
    int have_fingerprint = (read_p2_pgm("legacy-experiments/fingerprint.pgm", raw_fingerprint, &pw, &ph) == 0);
    int have_dense = (read_p2_pgm("legacy-experiments/live_dense_pad.pgm", raw_dense, &pw, &ph) == 0);
    int have_dense68 = (read_p2_pgm("legacy-experiments/live_dense_pad_seed68.pgm", raw_dense68, &pw, &ph) == 0);
    int have_clear = (read_p2_pgm("legacy-experiments/clear-0.pgm", raw_clear, &pw, &ph) == 0);
    if (!have_fingerprint) fprintf(stderr, "[warn] legacy-experiments/fingerprint.pgm missing, skipping\n");
    if (!have_dense) fprintf(stderr, "[warn] legacy-experiments/live_dense_pad.pgm missing, skipping\n");
    if (!have_dense68) fprintf(stderr, "[warn] legacy-experiments/live_dense_pad_seed68.pgm missing, skipping\n");
    if (!have_clear) fprintf(stderr, "[warn] legacy-experiments/clear-0.pgm missing, skipping\n");
    if (!have_dense) {
        fprintf(stderr, "[INCONCLUSIVE-because-template-frame-missing] genuine frame absent\n");
        return 1;
    }

    // ---- Load ticket-72 master template (finger A) ----
    FILE *tf = fopen("legacy-experiments/milan_dense_pad.tpl", "rb");
    if (!tf) {
        fprintf(stderr, "[INCONCLUSIVE-because-template-missing] legacy-experiments/milan_dense_pad.tpl absent\n");
        return 1;
    }
    fseek(tf, 0, SEEK_END);
    long tpl_len = ftell(tf);
    fseek(tf, 0, SEEK_SET);
    u8 *tpl_buf = malloc(tpl_len);
    if (!tpl_buf || fread(tpl_buf, 1, tpl_len, tf) != (size_t)tpl_len) {
        fprintf(stderr, "[INCONCLUSIVE-because-template-unreadable] short read\n");
        if (tf) fclose(tf);
        return 1;
    }
    fclose(tf);
    ...
    fprintf(stderr, "[unpack] templateUnPack(%ld bytes) returned %d, handle=%p\n",
            tpl_len, unpack_res, gallery);
    ...
    // ---- Synthetic white-noise probe (8-bit direct, variant-independent) ----
    ...
    // ---- Variant sweep ----
    Variant variants[] = {
        { "lc-g1.0-m128 (driver as-shipped)", 1.0f, 128.0f, 0 },
        { "lc-g1.5-m128 (shootout)",          1.5f, 128.0f, 0 },
        { "lc-g2.0-m128",                     2.0f, 128.0f, 0 },
        { "lc-g1.5-m100",                     1.5f, 100.0f, 0 },
        { "lc-g1.5-m160",                     1.5f, 160.0f, 0 },
        { "raw-shift>>4 (no local contrast)", 0.0f,   0.0f, 1 },
    };
    int n_variants = sizeof(variants) / sizeof(variants[0]);

    struct {
        const char *frame_name;
        u16 *raw;       // NULL for synthetic noise
        u8 *direct8;    // non-NULL for synthetic noise
        int present;
        int expect_match; // 1 = genuine finger A, 0 = impostor/blank/noise
    } frames[] = {
        { "live_dense_pad (genuine A)",          raw_dense,       NULL,      have_dense,       1 },
        { "live_dense_pad_seed68 (impostor B)",  raw_dense68,     NULL,      have_dense68,     0 },
        { "fingerprint.pgm (sparse impostor)",   raw_fingerprint, NULL,      have_fingerprint, 0 },
        { "clear-0 (blank)",                     raw_clear,       NULL,      have_clear,       0 },
        { "white noise (synthetic)",             NULL,            img_noise, 1,                0 },
    };
    int n_frames = sizeof(frames) / sizeof(frames[0]);

    fprintf(stderr, "\n=== Dynamic-range table: variant x frame -> quality/overlap/qout/match ===\n");
    fprintf(stderr, "%-34s | %-32s | q ov | qout[0] qout[1] | score | match | det | exp | status\n",
            "frame", "variant");
    fprintf(stderr, "-----------------------------------+----------------------------------+------+---------------+-------+-------+-----+-----+--------\n");

    u8 shaped[GOODIX_FRAME_SIZE];
    int genuine_ok = 1, impostor_ok = 1, raw_control_broke = 0;
    // Spread tracking on the midpoint-128 gain sweep (genuine vs blank/noise):
    int dense_q_at_gain[3] = {-1, -1, -1};
    int blank_q_at_gain[3] = {-1, -1, -1};
    int noise_q_at_gain[3] = {-1, -1, -1};

    for (int fi = 0; fi < n_frames; fi++) {
        if (!frames[fi].present) continue;
        for (int vi = 0; vi < n_variants; vi++) {
            u8 *input = shaped;
            float rmin = 0, rmax = 0;
            if (frames[fi].raw == NULL) {
                input = frames[fi].direct8; // noise: variant-independent
            } else if (variants[vi].is_raw) {
                raw12_to_8bit(frames[fi].raw, shaped);
            } else {
                normalize_gain_midpoint(frames[fi].raw, shaped,
                                        variants[vi].gain, variants[vi].midpoint,
                                        &rmin, &rmax);
            }

            GoodixImage probe;
            make_goodix_image(&probe, input, sensor_type);
            u32 qout[2] = {0xdeadbeef, 0xdeadbeef};
            getQuality(&probe, qout);

            GoodixImage probe2;
            make_goodix_image(&probe2, input, sensor_type);
            ...
            int correct = (is_match == frames[fi].expect_match);
            // FAR is evaluated over local-contrast shapings only (the only
            // inputs the driver plausibly feeds Milan). The raw-shift row is
            // a control proving normalization is required for matching.
            if (variants[vi].is_raw) { if (!correct) raw_control_broke = 1; }
            else if (frames[fi].expect_match) { if (!correct) genuine_ok = 0; }
            else { if (!correct) impostor_ok = 0; }

            fprintf(stderr, "%-34.34s | %-32.32s | %2u %2u | %7u %7u | %5d | %-5s | [%2u,%2u] | %d | %s\n",
                    frames[fi].frame_name, variants[vi].name,
                    probe.quality, probe.overlap, qout[0], qout[1],
                    match_score >= 0 ? match_score : 0,
                    is_match ? "MATCH" : "NO",
                    details[0], details[1],
                    frames[fi].expect_match, correct ? "PASS" : "FAIL");

            // Spread bookkeeping (midpoint-128 gains only: vi 0..2)
            if (vi <= 2 && !variants[vi].is_raw) {
                if (frames[fi].raw == raw_dense) dense_q_at_gain[vi] = probe.quality;
                if (frames[fi].raw == raw_clear) blank_q_at_gain[vi] = probe.quality;
                if (frames[fi].raw == NULL) noise_q_at_gain[vi] = probe.quality;
            }
            // Residual-range side channel for the dynamic-range successor:
            if (frames[fi].raw != NULL && !variants[vi].is_raw)
                fprintf(stderr, "    [residual] %s @ %s: min=%+.1f max=%+.1f range=%.1f\n",
                        frames[fi].frame_name, variants[vi].name,
                        rmin, rmax, rmax - rmin);
        }
    }
    ...
    fprintf(stderr, "-----------------------------------+----------------------------------+------+---------------+-------+-------+-----+-----+--------\n");
    fprintf(stderr, "[gain sweep] dense quality @ g={1.0,1.5,2.0}: %d %d %d\n",
            dense_q_at_gain[0], dense_q_at_gain[1], dense_q_at_gain[2]);
    fprintf(stderr, "[gain sweep] blank quality @ g={1.0,1.5,2.0}: %d %d %d\n",
            blank_q_at_gain[0], blank_q_at_gain[1], blank_q_at_gain[2]);
    fprintf(stderr, "[gain sweep] noise quality @ g={1.0,1.5,2.0}: %d %d %d\n",
            noise_q_at_gain[0], noise_q_at_gain[1], noise_q_at_gain[2]);
    fprintf(stderr, "[far] genuine-100 intact (lc variants): %s; impostor/blank/noise-0 intact (lc variants): %s; raw-control broke match (expected): %s\n",
            genuine_ok ? "yes" : "NO", impostor_ok ? "yes" : "NO",
            raw_control_broke ? "yes" : "no");

    int spread = 0;
    for (int vi = 0; vi < 3; vi++)
        if (dense_q_at_gain[vi] >= 10 && blank_q_at_gain[vi] == 0 && noise_q_at_gain[vi] == 0)
            spread = 1;

    templateDelete(gallery);
    free(tpl_buf);

    if (genuine_ok && impostor_ok && spread) {
        fprintf(stderr, "\n[VERDICT: CONFIRMED] shaping with quality spread found; FAR intact on lc variants%s.\n",
                raw_control_broke ? " (raw control breaks genuine match: normalization required)" : "");
        return 0;
    }
    if (genuine_ok && impostor_ok) {
        fprintf(stderr, "\n[VERDICT: FALSIFIED] getQuality flat across all shapings while identifyImage separates 100-vs-0; quality is a dead end, pursue residual_range/dynamic-range metric.\n");
        return 0;
    }
    fprintf(stderr, "\n[VERDICT: FAR-TRADE] shaping moves match scores (genuine_ok=%d impostor_ok=%d); do NOT wire into driver without a wider gallery.\n",
            genuine_ok, impostor_ok);
    return 2;
}
