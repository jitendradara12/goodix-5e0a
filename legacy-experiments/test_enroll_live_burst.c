...
// ================= TICKET 79 LIVE-TEMPLATE ENROLL + SCORE =================
// Offline only, no driver changes. Same pristine head (PE loader + PGM).
//
// Enrolls a Milan template from the Step-0 LIVE press burst (real live data,
// 4 frames = 4 touches, driver shaping lc-g1.0-m128, no synthetic
// perturbation), saves the packed template outside the repo, then scores
// every live frame (press + tap) and every synthetic ref vs the LIVE
// template: per-frame residual_range + getQuality + identifyImage score.
//
// Why: the argv-less probe run showed live ranges 702..935 (all above
// sparse 394 / blank 0) but zero live matches vs the synthetic finger-A
// template (different finger — expected NO). Matchability ("keeps matchable
// frames") can only be judged vs an own-finger live template. Tap-vs-press
// tells us behaviorally whether both bursts are the same finger.
//
// Usage:
...
int main(int argc, char **argv) {
    ...
    fprintf(stderr, "=== Ticket 79b: Live-Template Enroll + Score (offline) ===\n");
    ...
    if (!getAlgorithmVersion || !ppp_param_init || !getQuality || !enrolStartEx ||
        !enrolAddImage || !enrolGetTemplate || !enrolFinish || !templateGetPackedSize ||
        !templatePack || !templateUnPack || !templateDelete || !identifyImage) {
    ...
    if (strcmp(mode, "densectrl") == 0) {
        ...
        if (read_p2_pgm("legacy-experiments/live_dense_pad.pgm", raw_dense, &pw, &ph) != 0) {
            fprintf(stderr, "[INCONCLUSIVE-because-live-missing] dense base unreadable\n");
            return 1;
        }
        u8 base[GOODIX_FRAME_SIZE];
        { float rmin = 0, rmax = 0; normalize_driver_shaping(raw_dense, base, &rmin, &rmax, gain); }
        for (int step = 0; step < 8; step++) {
            int dx = (step % 3) - 1, dy = (step / 3) - 1;
            for (int y = 0; y < GOODIX_HEIGHT; y++)
                for (int x = 0; x < GOODIX_WIDTH; x++) {
                    ...
                    enrol_imgs[step][y * GOODIX_WIDTH + x] = base[sy * GOODIX_WIDTH + sx];
                }
        }
        n_enrol = 8;
        fprintf(stderr, "[enroll-src] densectrl: dense base + 8 shifted touches @ g%.1f\n", gain);
    } else {
        ...
        for (int c = 0; c < cycles; c++) {
            for (int i = 0; i < 4; i++) {
                u16 raw[GOODIX_FRAME_SIZE];
                int pw = 0, ph = 0;
                if (read_p2_pgm(press_paths[i], raw, &pw, &ph) != 0) {
                    fprintf(stderr, "[INCONCLUSIVE-because-live-missing] %s unreadable\n", press_paths[i]);
                    return 1;
                }
                float rmin = 0, rmax = 0;
                ...
                u32 qout[2] = {0xdeadbeef, 0xdeadbeef};
                getQuality(&qimg, qout);
                fprintf(stderr, "[enroll-src] %s range=%.1f q=%u ov=%u (pre-enroll)\n",
                        press_paths[i], rmax - rmin, qimg.quality, qimg.overlap);
                n_enrol++;
            }
        }
    ...
    int max_images = 16;
    void *ctx = enrolStartEx(&max_images);
    if (!ctx) { fprintf(stderr, "[INCONCLUSIVE-because-enroll-start-null]\n"); return 1; }
    *(u16*)((char*)ctx + 8) = 8;
    for (int t = 0; t < n_enrol; t++) {
        GoodixImage eimg;
        make_goodix_image(&eimg, enrol_imgs[t], sensor_type);
        u32 st[2] = {0};
        int add_res = enrolAddImage(ctx, &eimg, NULL, NULL, 0, st);
        int cnt = *(u16*)((char*)ctx + 10);
        int prog = *(int*)((char*)ctx + 12);
        fprintf(stderr, "[touch %d/%d] add_res=%d stitched=%d progress=%d%% q_status=[%u,%u]\n",
                t + 1, n_enrol, add_res, cnt, prog, st[0], st[1]);
    }
    void *master = NULL;
    int t_res = enrolGetTemplate(ctx, &master);
    fprintf(stderr, "[enrol] enrolGetTemplate=%d handle=%p\n", t_res, master);
    if (t_res != 0 || !master) { fprintf(stderr, "[INCONCLUSIVE-because-enroll-empty]\n"); return 1; }
    int packed_size = templateGetPackedSize(master);
    u8 *packed = malloc(packed_size);
    int pack_res = templatePack(master, packed);
    fprintf(stderr, "[enrol] packed=%d bytes pack_res=%d\n", packed_size, pack_res);
    if (pack_res != 0) return 1;
    // Binary outside the repo per env policy (pgm sources are gitignored anyway).
    char tplpath[128];
    snprintf(tplpath, sizeof(tplpath), "/tmp/opencode/live_%s_g%.1f.tpl", mode, gain);
    FILE *of = fopen(tplpath, "wb");
    if (of) { fwrite(packed, 1, packed_size, of); fclose(of); }
    fprintf(stderr, "[enrol] saved %s (%d bytes)\n", tplpath, packed_size);
    enrolFinish(ctx);

    void *gallery = NULL;
    int unpack_res = templateUnPack(packed, packed_size, NULL, &gallery);
    fprintf(stderr, "[unpack] roundtrip=%d handle=%p\n", unpack_res, gallery);
    if (unpack_res != 0 || !gallery) return 1;
    void *active_templates[1] = { gallery };

    // ---- Score matrix: all live frames + synthetic refs vs the template ----
    // Expectation depends on mode: live/live8 enroll press, so press+tap rows
    // are genuine-iff-same-finger (reported, not gated); densectrl enrolls
    // dense, so the dense row is the expected genuine self-match.
    static const char *score_paths_fixed[] = {
        "legacy-experiments/live_burst_press_01.pgm",
        "legacy-experiments/live_burst_press_02.pgm",
        "legacy-experiments/live_burst_press_03.pgm",
        "legacy-experiments/live_burst_press_04.pgm",
        ...
        "legacy-experiments/live_burst_tap_03.pgm",
        "legacy-experiments/live_burst_tap_04.pgm",
        "legacy-experiments/live_dense_pad.pgm",
        "legacy-experiments/live_dense_pad_seed68.pgm",
        "legacy-experiments/fingerprint.pgm",
        "legacy-experiments/clear-0.pgm",
        NULL,
    };
    // Score list = argv prefix frames first (ticket-80 burst, _01.._04 order),
    // then the fixed list minus duplicates (missing files warn+skip below).
    int is_densectrl = (strcmp(mode, "densectrl") == 0);
    const char *score_paths[24];
    int n_score = 0;
    {
        // Owned copies: prefix strings must outlive the scoring loop.
        static char owned[4][256];
        for (int i = 0; i < 4; i++) {
            snprintf(owned[i], sizeof(owned[i]), "%s_%02d.pgm", prefix, i + 1);
            int dup = 0;
            for (int k = 0; score_paths_fixed[k]; k++)
                if (strcmp(owned[i], score_paths_fixed[k]) == 0) { dup = 1; break; }
            if (!dup) score_paths[n_score++] = owned[i];
        }
        for (int k = 0; score_paths_fixed[k] && n_score < 23; k++)
            score_paths[n_score++] = score_paths_fixed[k];
        score_paths[n_score] = NULL;
    }
    u8 noise[GOODIX_FRAME_SIZE];
    srand(12345);
    for (int i = 0; i < GOODIX_FRAME_SIZE; i++) noise[i] = (u8)(rand() % 256);

    fprintf(stderr, "\n=== Score vs %s template @ g%.1f: frame -> range / q ov / score match ===\n",
            is_densectrl ? "DENSE-control" : "LIVE press", gain);
    // Caveat: quality below is read post-enrollment; engine state pollution
    // is observed (noise q=100 here vs 0/0 pre-enroll in ticket-78). Trust
    // only the pre-enroll [enroll-src] quality values and the match scores.
    u8 shaped[GOODIX_FRAME_SIZE];
    int live_match = 0, live_total = 0, neg_reject = 0, neg_total = 0;
    int ctrl_self_ok = 0;
    for (int si = 0; score_paths[si]; si++) {
        u16 raw[GOODIX_FRAME_SIZE];
        int pw = 0, ph = 0;
        if (read_p2_pgm(score_paths[si], raw, &pw, &ph) != 0) { fprintf(stderr, "[warn] %s missing\n", score_paths[si]); continue; }
        int active = 0;
        for (int i = 0; i < GOODIX_FRAME_SIZE; i++) if (raw[i] > 30) active++;
        float rmin = 0, rmax = 0;
        normalize_driver_shaping(raw, shaped, &rmin, &rmax, gain);
        GoodixImage probe; make_goodix_image(&probe, shaped, sensor_type);
        u32 qout[2] = {0xdeadbeef, 0xdeadbeef};
        getQuality(&probe, qout);
        GoodixImage probe2; make_goodix_image(&probe2, shaped, sensor_type);
        int midx = -999, mscore = -999; u32 det[2] = {0};
        identifyImage(&probe2, NULL, active_templates, 1, &midx, &mscore, det, 0, 0, NULL, 0);
        int is_match = (midx == 0 && mscore > 0);
        // Name-based populations (index-free: the list now varies with the
        // argv prefix and press2 rows). densectrl: the dense-base row is the
        // genuine self-match; every live_burst_* row is live population.
        int is_live = (strstr(score_paths[si], "live_burst_") != NULL);
        int is_dense_self = (strcmp(score_paths[si], "legacy-experiments/live_dense_pad.pgm") == 0);
        if (is_densectrl && is_dense_self) { if (is_match) ctrl_self_ok = 1; }
        else if (is_live) { live_total++; if (is_match) live_match++; }
        else { neg_total++; if (!is_match) neg_reject++; }
        fprintf(stderr, "%-46.46s | range=%7.1f active=%4d | q=%2u ov=%2u | score=%5d %-5s | [%2u,%2u]\n",
                score_paths[si], rmax - rmin, active, probe.quality, probe.overlap,
                mscore >= 0 ? mscore : 0, is_match ? "MATCH" : "NO", det[0], det[1]);
    }
    // Synthetic noise vs live template (impostor control).
    {
        GoodixImage probe; make_goodix_image(&probe, noise, sensor_type);
        GoodixImage probe2; make_goodix_image(&probe2, noise, sensor_type);
        int midx = -999, mscore = -999; u32 det[2] = {0};
        identifyImage(&probe2, NULL, active_templates, 1, &midx, &mscore, det, 0, 0, NULL, 0);
        int is_match = (midx == 0 && mscore > 0);
        neg_total++; if (!is_match) neg_reject++;
        fprintf(stderr, "%-46.46s | range=%7s active=%4s | q=%2u ov=%2u | score=%5d %-5s | [%2u,%2u]\n",
                "white noise (synthetic)", "-", "-", probe.quality, probe.overlap,
                mscore >= 0 ? mscore : 0, is_match ? "MATCH" : "NO", det[0], det[1]);
    }
    fprintf(stderr, "[result] live: %d/%d MATCH; non-live reject: %d/%d%s\n",
            live_match, live_total, neg_reject, neg_total,
            is_densectrl ? "; control self-match (dense-vs-dense): ok" : "");
    if (is_densectrl)
        fprintf(stderr, "[control] dense self-match=%s packed=%dB (ticket-72 template 23110B)\n",
                ctrl_self_ok ? "yes-100" : "NO", packed_size);
    if (!is_densectrl && live_match == live_total && live_total > 0 && neg_reject == neg_total)
        fprintf(stderr, "[VERDICT: CONFIRMED] live frames match own template, all non-live rejected; range column above is the rank signal.\n");
    else if (!is_densectrl && live_match == 0)
        fprintf(stderr, "[VERDICT: FALSIFIED] residual_range cannot rank matchability: live ranges (rows above) meet/exceed matchable dense 727 yet enroll/add_res and match 0/%d; do NOT wire.\n", live_total);
    else if (is_densectrl)
        fprintf(stderr, "[VERDICT: CONTROL-OK] enroll path healthy (8/8 stitched @ g%.1f); live rejection is frame content, not harness.\n", gain);
    else
        fprintf(stderr, "[VERDICT: PARTIAL] some live match, some reject — inspect per-frame rows before any wiring.\n");
