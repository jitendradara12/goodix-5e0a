...
int main() {
    ...
    for (int i = 0; i < 5120; i++) {
        ...
        norm1[i] = (unsigned char)(v < 0 ? 0 : (v > 255 ? 255 : v));
    }
    test_pipeline(norm1, "Method 1: min_nz clipping");

    unsigned char norm2[5120];
    for (int i = 0; i < 5120; i++) {
        int v = (int)((vals[i] * 255.0f) / max_v);
        norm2[i] = (unsigned char)(v < 0 ? 0 : (v > 255 ? 255 : v));
    }
    test_pipeline(norm2, "Method 2: zero baseline preserved");
