...
class TestF14FrameTLSDecrypt(unittest.TestCase):
    ...
    def test_tls_decrypted_frame_payload_trimming(self):
        """Verify 7,684-byte decrypted TLS buffer trims trailing 4 checksum/status bytes to yield 7,680B."""
        raw_7680 = b"\x12" * 7680
        tls_payload = raw_7680 + b"\x00\x00\x00\x00"
        self.assertEqual(len(tls_payload), 7684)
        trimmed = tls_payload[:-4] if len(tls_payload) == 7684 else tls_payload
        self.assertEqual(len(trimmed), 7680)
